...
STRATEGY_VERSION = "2.0.0-optimized"
ADJUST_TYPE = "1"
...
def _parse_args():
    ...
    parser.add_argument("--higher-period", default="60m", choices=("60m", "120m"))
    parser.add_argument("--output-dir", default=str(RUNS_ROOT / "single_symbol" / "optimized"))
    ...
    parser.add_argument("--vwap-minutes", type=int, default=60)
...
def _freeze_dataset(
    symbol, start_date, end_date, kline_period, output_dir, name,
    fetch_start=None, evaluation_start=None,
):
    ...
    if df is None or df.empty:
        raise RuntimeError(
            f"SSQuant returned no {symbol} {kline_period} bars; "
            "formal run stopped instead of invoking an unrecorded fallback"
        )
    ...
    evaluation_hash = bars_hash
    evaluation_rows = len(frozen)
    if evaluation_start:
        evaluation = frozen[pd.to_datetime(frozen["datetime"]) >= pd.Timestamp(evaluation_start)]
        evaluation_payload = evaluation.to_csv(
            index=False, lineterminator="\n", float_format="%.15g"
        ).encode("utf-8")
        evaluation_hash = hashlib.sha256(evaluation_payload).hexdigest()
        evaluation_rows = len(evaluation)
    freeze_dir = output_dir / "frozen_dataset"
    freeze_dir.mkdir(parents=True, exist_ok=True)
    bars_path = freeze_dir / f"{name}_bars.csv"
    bars_path.write_bytes(payload)
    manifest = {
        "symbol": symbol,
        "kline_period": kline_period,
        "start_date": start_date,
        "fetch_start": fetch_start or start_date,
        "evaluation_start": evaluation_start or start_date,
        "end_date": end_date,
        "adjust_type": ADJUST_TYPE,
        ...
        "evaluation_bars_sha256": evaluation_hash,
        "bars_file": str(bars_path),
        "frozen_at": datetime.now().isoformat(timespec="seconds"),
    }
    (freeze_dir / f"{name}_dataset_manifest.json").write_text(
...
def _load_frozen_bars(manifest):
    frame = pd.read_csv(manifest["bars_file"], parse_dates=["datetime"])
    frame = frame.set_index("datetime").sort_index()
    return frame


def _install_frozen_data(runner, symbol, small_dataset, higher_dataset):
    from ssquant.backtest.backtest_data import BacktestDataManager

    frozen = {
        f"{symbol}_{small_dataset['kline_period']}_{ADJUST_TYPE}": _load_frozen_bars(small_dataset),
        f"{symbol}_{higher_dataset['kline_period']}_{ADJUST_TYPE}": _load_frozen_bars(higher_dataset),
    }

    def fetch_frozen(_manager, _symbols_and_periods, _symbol_configs, _base_config):
        return {key: value.copy() for key, value in frozen.items()}

    # Keep all subsequent construction inside SSQuant; replace only its data
    # acquisition boundary with the already frozen, hash-audited frames.
...
def _period_minutes(period):
    return int(str(period).lower().rstrip("m"))
...
def main():
    ...
    warmup_start = (
        pd.Timestamp(args.start_date) - pd.Timedelta(days=30)
    ).strftime("%Y-%m-%d")
    small_dataset = _freeze_dataset(
        args.symbol, args.start_date, args.end_date, args.kline_period, output_dir, "small",
        fetch_start=warmup_start, evaluation_start=args.start_date,
    )
    higher_dataset = _freeze_dataset(
        args.symbol, args.start_date, args.end_date, args.higher_period, output_dir, "higher",
        fetch_start=warmup_start, evaluation_start=args.start_date,
    )
    # align_data shifts the higher timeframe forward by one bar so that its
    # value represents a closed bar. Start the engine at that first close;
    # leading NaN values must not be filled with future data.
    execution_start_time = (
        pd.Timestamp(higher_dataset["first_datetime"])
        + pd.Timedelta(minutes=_period_minutes(args.higher_period))
    ).strftime("%Y-%m-%d %H:%M:%S")
    dataset = {
        "small": small_dataset,
        "higher": higher_dataset,
        "bars_sha256": small_dataset["bars_sha256"],
        "higher_bars_sha256": higher_dataset["bars_sha256"],
        "higher_evaluation_bars_sha256": higher_dataset["evaluation_bars_sha256"],
        "rows": small_dataset["rows"],
        "evaluation_bars_sha256": small_dataset["evaluation_bars_sha256"],
        "evaluation_rows": small_dataset["evaluation_rows"],
    ...
    strategy_params = {
        ...
        "higher_period_minutes": _period_minutes(args.higher_period),
        "evaluation_start": args.start_date,
    ...
    config = get_config(
        ...
        data_sources=[
            ...
            {
                "symbol": args.symbol,
                "kline_period": args.higher_period,
                "adjust_type": ADJUST_TYPE,
                "initial_capital": args.initial_capital,
            },
        ],
    ...
    data_manager_class, original_fetch = _install_frozen_data(
        runner, args.symbol, small_dataset, higher_dataset
    )
    try:
        results = runner.run(
    ...
    report_md.write_text(
        "\n".join(
            [
                ...
                f"| Evaluation period | {args.start_date} to {args.end_date} |",
                f"| Warmup data start | {warmup_start} |",
                f"| Execution start | {execution_start_time} (first closed higher bar) |",
                f"| K-line | {args.kline_period}; higher timeframe {args.higher_period} |",
                f"| Adjustment | `adjust_type={ADJUST_TYPE}` |",
                f"| Frozen rows | {dataset['rows']} |",
                f"| Bars SHA-256 | `{dataset['bars_sha256']}` |",
                f"| Higher bars | {dataset['higher']['rows']} |",
                f"| Higher SHA-256 | `{dataset['higher_bars_sha256']}` |",
                f"| Trades | {len(trades)} |",
    ...
    manifest = {
        "formal": True,
        "symbol": args.symbol,
        "bars_hash": dataset["evaluation_bars_sha256"],
        ...
        "adjustment_mode": "ssquant_adjust_type_1",
        "data_source": "ssquant",
        "data_pipeline": "SSQuant official get_futures_data -> frozen CSV -> SSQuant BacktestDataManager/DataSource",
        "frozen_input_used": True,
        "mtf_alignment": "SSQuant official multi-data-source API; align_data=False; strategy only uses higher bars whose close time is <= current base bar",
        "fallback_forbidden": True,
        "strategy": "microstructure-vwap-deviation-optimized",
    ...
    summary = {
        ...
        "time_frame": {"base": args.kline_period, "higher": args.higher_period},
